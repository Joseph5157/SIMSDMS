    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent
        className={cn(
          ...
          'border border-slate-200 ring-0 shadow-xl'
        )}
      ...
      >
        <DialogHeader className="px-7 py-5 border-b border-slate-100 shrink-0">
          <DialogTitle id={titleId} className="text-base font-semibold text-slate-900 leading-snug">
            {title}
          </DialogTitle>
          <DialogDescription id={descriptionId} className={description ? "text-[13px] text-slate-500 mt-0.5" : "sr-only"}>
            {description || title}
          </DialogDescription>
        </DialogHeader>
        ...
        {footer && (
          <div className="shrink-0 px-7 py-4 flex justify-end gap-3 border-t border-slate-100 bg-slate-50/50">
            {footer}
          </div>
        )}
      </DialogContent>
