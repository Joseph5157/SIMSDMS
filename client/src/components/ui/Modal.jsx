...
export default function Modal({ open, onClose, title, description, size = 'md', children, footer }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      ...
      >
        <DialogHeader className="px-4 sm:px-6 md:px-8 py-4 md:py-6 border-b border-slate-200 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-4 sm:px-6 md:px-8 py-4 md:py-6 flex flex-col gap-4">
          {children}
        </div>
        {footer && (
          <div className="shrink-0 px-4 sm:px-6 md:px-8 py-3 md:py-4 flex justify-end gap-2 border-t border-slate-200 bg-white">
            {footer}
          </div>
        )}
      </DialogContent>
